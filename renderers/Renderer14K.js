class Renderer14K {
    ...
    render(time) {
        ...
        const drawSide = (offsetX, isP2) => {
            ...
            // Per-lane drawer
            const drawSingleLane = (actList, x, w, isBlack, isBlue, isScratch, beamIndex) => {
                const actionList = Array.isArray(actList) ? actList : [actList];
                const active = actionList.some(a => state.activeActions.has(a));
                // Note: beamIndex needs to be unique globally or per side? 
                // state.beamOpacity is a single array. We need to offset indices for P2.
                // P1: 0-7. P2: 8-15.
                ...
                if ((state.beamOpacity[realBeamIdx] || 0) > 0.01) {
            ...
            // Draw Structure
            if (!isP2) {
                // P1: Scratch | 1 2 3 4 5 6 7
                drawSingleLane([actions.P1_SC_CCW, actions.P1_SC_CW], 0, wScratch, false, false, true, 0);
                let cx = wScratch;
                for (let i = 1; i <= 7; i++) {
                    const isBlue = (i === 2 || i === 4 || i === 6);
                    drawSingleLane(actions[`P1_${i}`], cx, wKey, !isBlue, isBlue, false, i);
                    cx += wKey;
                }
            } else {
                // P2: 1 2 3 4 5 6 7 | Scratch
                let cx = 0;
                for (let i = 1; i <= 7; i++) {
                    const isBlue = (i === 2 || i === 4 || i === 6);
                    // Beam index 1-7 (mapped to 9-15)
                    drawSingleLane(actions[`P2_${i}`], cx, wKey, !isBlue, isBlue, false, i);
                    cx += wKey;
                }
                // Scratch at end (Index 0 mapped to 8)
                drawSingleLane([actions.P2_SC_CCW, actions.P2_SC_CW], cx, wScratch, false, false, true, 0);
            }
